		...
		return this.each(function() {
			...
			var $input = $('<input type="text" class="multiple_emails-input text-left" placeholder="' + settings.placeholder +'"/>').on('keyup', function(e) { // input
				...
				if(keynum == 9 || keynum == 32 || keynum == 188) {
					display_email($(this), settings.checkDupEmail, settings.validate);
				}
				else if (keynum == 13) {
					display_email($(this), settings.checkDupEmail, settings.validate);
				...
				if ($(this).val() != '') { display_email($(this), settings.checkDupEmail, settings.validate); }
			});
			...
			*/
			function display_email(t, dupEmailCheck, validate) {
